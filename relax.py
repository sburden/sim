...
class BB(HDS):
  ...
  def P(self, g=1., c=1., s=0.1, debug=False):
    """
    Parameters:
      g - scalar - gravitational constant
      c - scalar - coefficient of restitution
      s - scalar - stochasticity parameter
      debug - bool - flag for printing debugging info
    """
    return Struct(j=1,g=g,c=c,s=s,debug=debug)
...
def stack(trjs, J=np.arange(100)):
  """
...
def trans(trjs, J=np.arange(100)):
  """
  ...
  for k in range(len(trjs)):
    if np.any(trjs[k].p.j == J) and len(trjs[k].t) >= 3:
...
def descent1(f, x0, D=lambda f,x,fx : jac(f,x,fx=fx), 
             eps=1e-4, ftol=-np.inf, d=None, a=armijo, 
  ...
  Dfx = D(f,x,fx)
  if debug:
    print '(descent1)             x = %s, f(x) = %0.2e' % (x,fx)
    print '                   Df(x) = %s' % (Dfx)
    sys.stdout.flush()
...
if __name__ == "__main__":
  ...
  do_descent1 = False
  #do_descent1 = True
  do_descent1proj = False
  do_descent1proj = True
  ...
  if do_descent1:
    ...
    n = 5
    D = np.diag( np.logspace( -2,1,n ) )
    #D = np.identity(5)
    F = lambda x : np.dot(x[:,np.newaxis].T,np.dot( D, x[:,np.newaxis] ) )[0,0]

    if 0:
      #x0 = np.dot(np.random.randn(n),np.linalg.inv(D)) # makes descent1 fail
      x0 = np.dot(np.random.randn(n),D)
      #x0 = np.array([0.25683403,-0.60790682,-0.70658859,-0.86598863,-1.41549134])
      x,j, = descent1(F,x0,debug=False,jmax=1000)
      #print 'x0 = %s' % x0
      #print 'x  = %s, j = %s' % (x,j)
      print '|x0| = %0.2e' % np.sum(x0*x0)
      print '|x|  = %0.2e, j = %s' % (np.sum(x*x),j)

    else:
      X0 = np.random.randn(100,10)
      X = np.array([descent1(f,x0)[0] for x0 in X0])
      plt.figure(1); plt.clf()
      
      plt.hist( np.sum( X*X,axis=1) )
      print 'nans @ %s' % np.isnan(np.sum(X,axis=1)).nonzero()[0]
  ...
  if do_descent1proj:
    ...
    if 1:
      ...
      x0 = np.dot(np.random.randn(n),D)
      #x0 = np.array([0.25683403,-0.60790682,-0.70658859,-0.86598863,-1.41549134])
      x,j, = descent1proj(F,x0,debug=debug,
                          Pi=lambda x : np.array([x[0],x[0]**2+1.]),
                          TM=lambda x : np.array([1,2*x[0]]))
      #print 'x0 = %s' % x0
      #print 'x  = %s, j = %s' % (x,j)
      print '|x0| = %0.2e' % np.sum(x0*x0)
      print '|x|  = %0.2e, j = %s' % (np.sum(x*x),j)
