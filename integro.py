...
#
# I derived the code from a MEX interface to dopri5 written by Madhu ????
#
# $Log:$
...
from numpy import \
    ...
    zeros,concatenate,floating,tensordot,vstack,diff,c_,r_
...
class Integro(object):
  ...
  def solout (self,ni,xold, x, y, ny ):
  ...
  def dopcor (self,n, x, y, pars, xend, hmax, h, rtoler, atoler, itoler, fileout,
    ...
    self.hout = h
    self.xout = x
    if self.solout (self.naccpt + 1, self.xold, x, y, n):
      return -1
    while True:
      if (self.nstep > nmax):
        self.xout = x
  ...
  def dope(self, y0, tEnd, pars=[], dt=None, nmax=100000):
    ...
    return self.dopri5( 
      ...
      sys.stdout,       #  FILE* fileout,    messages stream 
  ...
  def dopri5 (self, n, x, y, pars,
    ...
    if (nrdens > n):
      ...
      self.rcont = zeros((5,nrdens),dtype=floating)
...
class odeDP5( Integro ):
  """Convenience interface to the Dopri5 integrator, allowing it to wrap a
    callable (e.g. lambda
  ...
  def __call__(self,y0,t0,t1=None,dt=None):
    """Integrate the ODE from initial condition y0 at time t0 to time t1
       ...
       returns t,y arrays of sample times and states
         AND   code returned from dope calling dopri5 calling dopcor
    """
    if t1 is None:
    ...
    y0 = concatenate( ((t0,), y0) )
    self.tOut = []
    self.yOut = []
    self.trj = None
    code = self.dope( asarray( y0, dtype=floating ), tEnd = t1, dt=dt )
    self.tOut = array(self.tOut)
    self.yOut = vstack(self.yOut)
    return self.tOut[:,1], self.yOut, code
...
if __name__=="__main__":
  ...
  print """Running unit tests:
             ...
             (1) Sample test -- chosen times, dense sampling
             (2) Event detector
             (3) Rossler system 
             (4) Order test
        """
